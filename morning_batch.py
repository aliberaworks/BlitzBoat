...
# Windows cp932 環境で絵文字が化けないようにUTF-8強制
...
def load_models():
    ...
    clf_arare = None
    if os.path.exists(MODEL_ARARE):
        with open(MODEL_ARARE, "rb") as f:
            clf_arare = pickle.load(f)
        print("荒れモデル読み込み完了")
    else:
        print("[INFO] 荒れモデルなし（train_arare_model.py を実行してください）")

    return clf_boat, clf_km, meta, clf_arare
...
# ── 荒れ警報スコア計算 ────────────────────────────────────────────────────────
def calc_arare_score(r: dict) -> float:
    """
    荒れ警報スコアを算出（高いほど荒れやすい）。
    ・1号艇の予測勝率が低い（= モデルが1号艇を本命視していない）
    ・1号艇がB級選手
    ・他の艇にA1選手がいる（外強豪）
    ・全体エントロピーが高い（誰が勝つか不明な混戦）
    """
    boat_prob = {int(k): v for k, v in r["boat_prob"].items()}
    boat_data = r.get("boat_data", {})

    p1 = boat_prob.get(1, 1 / 6)

    # 1号艇勝率が基準（1/6）より低いほど加点
    p1_score = max(0.0, (1 / 6 - p1) * 300)

    # エントロピー（確率が均等 = 混戦 = 最大値）
    H = -sum(p * math.log(p) for p in boat_prob.values() if p > 0)
    H_max = math.log(6)
    entropy_score = (H / H_max) * 40

    # 1号艇がA1/A2以外 → B級選手（intキー・strキー両対応）
    b1 = boat_data.get(1) or boat_data.get("1") or {}
    if not isinstance(b1, dict):
        b1 = {}
    b1_grade = b1.get("grade", "")
    grade_b1_score = 25.0 if b1_grade not in ("A1", "A2") else 0.0

    # 1号艇以外にA1選手が何人いるか
    a1_count = sum(
        1 for i in range(2, 7)
        for bi in [boat_data.get(i) or boat_data.get(str(i)) or {}]
        if isinstance(bi, dict) and bi.get("grade") == "A1"
    )
    grade_other_score = a1_count * 15.0

    return p1_score + entropy_score + grade_b1_score + grade_other_score
...
def fetch_and_predict_venue(
    ...
    for rno in range(1, n_races + 1):
        try:
            ...
            # 荒れ確率（モデルがあれば使う、なければデフォルト0.5）
            arare_prob = 0.5
            if clf_arare is not None:
                try:
                    arare_prob = float(clf_arare.predict_proba(X)[0][1])
                except Exception:
                    pass
...
def main():
    ...
    # モデルロード
    clf_boat, clf_km, meta, clf_arare = load_models()
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futures = {
            ex.submit(fetch_and_predict_venue, jcd, name, hd, n_races, clf_boat, clf_km, km_by_boat,
                      bubble_map.get(jcd), clf_arare): jcd
