...
def format_arare_alert(arare_races: list, ymd: str) -> str:
    """
    荒れ警報メッセージを生成。
    arare_races: morning_batch.py が荒れスコア降順でソートした予測結果リスト。
    各要素に boat_prob / boat_data / top_boat / arare_score が必要。
    """
    lines = [
        f"🌪️ BlitzBoat 荒れ警報  {ymd}",
        f"1号艇が弱く外強豪あり or 混戦レース TOP{len(arare_races)}",
        "",
    ]

    for rank, r in enumerate(arare_races, 1):
        venue   = r.get("venue_name", "")
        rno     = r.get("race_no", 0)
        rtime   = r.get("race_time", "--:--")
        bprob   = {int(k): v for k, v in r["boat_prob"].items()}
        bdata   = r.get("boat_data", {})

        # 荒れ度を★で表現（arare_prob優先、なければarare_scoreを0〜1換算）
        arare_p = r.get("arare_prob", r.get("arare_score", 0) / 100)
        star_count = min(5, max(1, int((arare_p - 0.45) / 0.1)))
        stars = "★" * star_count + "☆" * (5 - star_count)

        p1 = bprob.get(1, 0.0)
        # boat_dataはPythonオブジェクト(intキー)またはJSON復元後(strキー)の両方を考慮
        b1 = bdata.get(1) or bdata.get("1") or {}
        if not isinstance(b1, dict):
            b1 = {}
        b1_grade = b1.get("grade", "?")

        # A1選手（1号艇以外）を列挙
        a1_others = []
        for i in range(2, 7):
            bi = bdata.get(i) or bdata.get(str(i)) or {}
            if isinstance(bi, dict) and bi.get("grade") == "A1":
                a1_others.append(f"{_BOAT_LABEL[i-1]}A1")

        top_boat = r.get("top_boat", 1)
        top_prob = bprob.get(top_boat, 0.0)

        line = (
            f"{rank}. {venue} {rno}R {rtime}  {stars}\n"
            f"   ①{b1_grade} {p1*100:.0f}%  本命:{_BOAT_LABEL[top_boat-1]} {top_prob*100:.0f}%"
        )
        if a1_others:
            line += f"  A1外: {' '.join(a1_others)}"

        lines.append(line)

    lines += ["", "⚠ 荒れ確定ではなく「可能性が高い」レースです"]
    return "\n".join(lines)


def send_arare_alert(arare_races: list, ymd: str) -> bool:
    """荒れ警報をLINEに送信。"""
    text = format_arare_alert(arare_races, ymd)
