...
ARARE_PROB_THRESH    = 0.60  # 荒れ確率がこの値以上のレースのみEV通知する
...
MAX_BETS_PER_RACE = 3   # 買い目の最大表示点数（backtest最適値）
...
def _format_consolidated(notify_list: list, now: datetime, ev_thresh: float, sent_today: int) -> str:
    ...
    for rank, n in enumerate(notify_list, 1):
        ...
        star_n    = min(5, max(1, int((arare_p - 0.50) / 0.05)))
...
def _compute_ev_arare(boat_prob, arare_prob, odds_3t, arare_stats, venue_name):
    ...
    for r1 in BOATS:
        ...
        cond = {pair: 0.0 for pair in all_r2r3}
...
def _compute_ev(boat_prob, odds_3t, meta):
...
def run(hd: str, win_min: int = 30, win_max: int = 60):
    ...
    for mins, p in sorted(targets, key=lambda x: x[0]):
        ...
        try:
            ...
            if top_ev:
                notify_list.append({
                    "race": p, "ev_rows": ev_rows,
                    "arare_prob": arare_prob, "course_changes": course_changes,
                })
